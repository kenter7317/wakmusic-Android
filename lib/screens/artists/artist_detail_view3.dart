...
class _ArtistViewState extends State<ArtistView> with TickerProviderStateMixin {
        ...
        children: [
          ...
          NestedScrollView(
              ...
              headerSliverBuilder: (context, isScrolled) => [
                    ...
                    //                     widget.artist.colors.length,
                    //                     (idx) => double.parse(
                    //                         widget.artist.colors[idx][2])),
                    //                 colors: List.generate(
                    //                     widget.artist.colors.length,
                    //                     (idx) =>
                    //                         Color(int.parse("0xFF${widget.artist.colors[idx][0]}"))
                    //                             .withOpacity(int.parse(widget
                    //                                     .artist
                    //                                     .colors[idx][1]) *
                    //                                 0.6 /
                    //                                 100)),
                    ...
                    //     expandedHeight: 88 + artistImgRatio * 180,
              ...
              body: albumsTabView(_tabController)),
        ],
      ),
    );
  }
  ...
  Widget artistInfo(BuildContext context) {
    ...
    return Padding(
      padding: EdgeInsets.fromLTRB(
          20, MediaQuery.of(context).viewPadding.top + 64, 20, 16),
      child: Column(
        children: [
          Row(
            children: [
                          ...
                          Expanded(
                            child: NotificationListener(
                              onNotification: ((notification) {
                                _descScrollController.addListener(() {
                                  setState(() {
                                    descScrollHeight =
                                        (_descScrollController.position.pixels /
                                                _descScrollController
  ...
  Widget albumsTabView(TabController tabController) {
    ArtistsViewModel viewModel = Provider.of<ArtistsViewModel>(context);
    double artistImgRatio = (MediaQuery.of(context).size.width - 48) / 327;
    if (viewModel.albums[AlbumType.values[_tabController.index]] == null) {
      ...
      return TabBarView(
          ...
          children: List.generate(
            ...
            (index) => ListView.builder(
              ...
              itemBuilder: (_, idx) {
                if (idx == viewModel.albums[AlbumType.values[index]]!.length) {
