...
class SearchView extends StatelessWidget {
      ...
      child: TextFormField(
        ...
        onFieldSubmitted: (keyword) {
          if (keyword.isNotEmpty) {
            viewModel.updateStatus(SearchStatus.after);
                    ...
                    child: Container(
                      ...
                      child: Center(
                        child: Text(
                          '취소',
                          style:
                              WakText.txt12B.copyWith(color: WakColor.grey400),
                        ),
                      ),
                    ),
                  ),
  ...
  Widget _buildAfter(BuildContext context) {
    ...
    return FutureBuilder<List<List<Song>>>(
      future: Future.wait(
          [viewModel.list[0], viewModel.list[1], viewModel.list[2]]),
                      ...
                      (idx) {
                        if (idx == 0) {
                          return snapshot.hasData
                              ? _buildTotalTab(context, snapshot.data!)
                              : const Center(
                                  child: CircularProgressIndicator(),
                                );
                        } else {
                          return snapshot.hasData
                              ? _buildTab(context, snapshot.data![idx - 1])
                              : const Center(
                                  child: CircularProgressIndicator(),
                                );
                        }
  ...
  Widget _buildTotalTab(BuildContext context, List<List<Song>> list) {
    ...
    if (tabs.isEmpty) {
      ...
      return ListView.builder(
        ...
        itemBuilder: (context, idx) {
          if (idx == 0) {
            return _buildTabHeader(context, tabs[0], list[tabs[0]].length);
          }
          return SongTile(
              song: list[tabs[0]][idx - 1], tileType: TileType.dateTile);
        },
      );
    } else {
      return ListView.separated(
        ...
        itemBuilder: (context, idx) {
          return Column(
            children: [
              _buildTabHeader(context, tabs[idx], list[tabs[idx]].length),
              Column(
                mainAxisSize: MainAxisSize.min,
                children: List.generate(3, (songIdx) {
  ...
  Widget _buildTabHeader(BuildContext context, int tabIdx, int length) {
    return Padding(
  ...
  Widget _buildTab(BuildContext context, List<Song> list) {
    if (list.isEmpty) {
      ...
      return ListView.builder(
        ...
        itemBuilder: (context, idx) {
          return SongTile(song: list[idx], tileType: TileType.dateTile);
        },
      );
    }
  }
