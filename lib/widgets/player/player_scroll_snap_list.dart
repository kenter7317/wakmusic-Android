...
class PlayerScrollSnapList extends StatefulWidget {
  ///List background
  ...
  @override
  PlayerScrollSnapListState createState() => PlayerScrollSnapListState();
}

class PlayerScrollSnapListState extends State<PlayerScrollSnapList> {
  ...
  double _calcCardLocation(
    ...
    int cardIndex = index ?? _calcCardIndex(pixel!);
    ...
    //target position
    return widget.positionList[cardIndex].toDouble();
  }

  int _calcCardIndex(double pixel){
    int target = pixel.toInt();

    int left = 0;
    int right = widget.positionList.length - 1;
    int index = 0;
    int position = widget.positionList[index];

    while (left <= right) {
      int mid = left + ((right - left) ~/ 2); // 중간 인덱스 계산

      if (widget.positionList[mid] == target) {
        return mid; // 일치하는 값이 있으면 바로 반환
      } else if (widget.positionList[mid] < target) {
        left = mid + 1;
      } else {
        right = mid - 1;
      }
      ...
      // 현재까지의 가장 가까운 값 갱신
      if ((widget.positionList[mid] - target).abs() < (position - target).abs()) {
        position = widget.positionList[mid];
        index = mid;
      }
    }
  ...
  void focusToItem(int index) {
    ...
    print('calc result : $targetLoc');
    _animateScroll(targetLoc);
  }

  ///Determine location if initialIndex is set
  void focusToInitialPosition() {
    widget.listController.jumpTo(widget.positionList[widget.initialIndex!].toDouble());
  }
